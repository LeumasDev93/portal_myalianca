...
export async function GET() {
    console.log("=== DEBUG API PRODUCTS LIST ===");
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: 'Failed to fetch products list', details: responseData },
                { status: response.status }
            );
        }

        console.log("Successfully returning products list");
        return NextResponse.json(responseData);
    } catch (error) {
        console.error("Error in API route:", error);
        return NextResponse.json(
            {
                error: 'Internal error while fetching products list',
